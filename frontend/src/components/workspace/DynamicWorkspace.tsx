...
// Extracted REAL data from the provided invoice JSON
const invoiceData = [
  {
    invoice: "Invoice 1",
    amount: 634.73,
...
export function DynamicWorkspace() {
  ...
  const totalExpenses = invoiceData.reduce((total, invoice) => total + invoice.amount, 0);
  const totalInvoices = invoiceData.length;
  const averageAmount = totalInvoices > 0 ? (totalExpenses / totalInvoices).toFixed(2) : 0;

  // Prepare data for LineChart (monthly trends)
  const monthlyData = Array.from({ length: 12 }, (_, index) => ({ month: new Date(0, index).toLocaleString('default', { month: 'short' }), total: 0 }));

  invoiceData.forEach(invoice => {
    const monthIndex = new Date(invoice.date).getMonth();
    monthlyData[monthIndex].total += invoice.amount;
  });
